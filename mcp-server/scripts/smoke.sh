...
#
# Drives one JSON-RPC session through: initialize -> query (SPARQL over the SEEDED fleet
# world, sent as the FIRST tools/call in the SAME write as the initialized notification —
# zero delay — proving the seed is in place before the server accepts requests, not racing
# it) -> tools/list -> resources/list -> prompts/list -> recall_memory (pattern recall over
# the seed) -> store_memory -> create_stream -> register_stream_query (a [NOW] speeding
# monitor) -> push_stream_events (three typed speed readings as RDF-Messages nquads) ->
# recall_memory(queryId) drain -> forget_stream_query.
...

{
  ...
  # -- race-proof check: the seeded-world SPARQL is the FIRST tools/call, written in the SAME
  #    printf as the initialized notification (no sleep between them) — the launcher seeds the
  #    RDF store BEFORE the server starts, so even a zero-delay first call must see the fleet --
  printf '%s\n' \
    '{"jsonrpc":"2.0","method":"notifications/initialized"}' \
    '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"query","arguments":{"query":"SELECT ?v ?d WHERE { ?v <https://covesa.global/fleet#assignedDriver> ?d }","language":"sparql"}}}'
  sleep 0.6
  # -- discovery: the full published surface ---------------------------------------------------
  emit '{"jsonrpc":"2.0","id":3,"method":"tools/list"}'
  emit '{"jsonrpc":"2.0","id":4,"method":"resources/list"}'
  emit '{"jsonrpc":"2.0","id":5,"method":"prompts/list"}'
  # -- the seed is also visible to recall_memory pattern recall (long-term memory graph) --------
...
assert_contains "FIRST zero-delay call sees the seed (alice)"  "alice"                          '"id":2'
assert_contains "FIRST zero-delay call sees the seed (bob)"    "bob"                            '"id":2'
assert_contains "FIRST zero-delay call sees the seed (carol)"  "carol"                          '"id":2'
assert_contains "tools/list includes store_memory"             '"name":"store_memory"'          '"id":3'
assert_contains "tools/list includes register_stream_query"    '"name":"register_stream_query"' '"id":3'
assert_contains "tools/list includes watch_invariant"          '"name":"watch_invariant"'       '"id":3'
assert_contains "resources/list includes engine status"        "cqels://engine/status"          '"id":4'
assert_contains "prompts/list is non-empty"                    '"name"'                         '"id":5'
