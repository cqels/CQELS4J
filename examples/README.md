...
`https://raw.githubusercontent.com/cqels/maven/main/releases`, which this project's POM already
points at — no account, no token, no `~/.m2/settings.xml`.
...
| [`DriverAttentionWatchdog`](src/main/java/org/cqels/examples/cdsp/DriverAttentionWatchdog.java) | CEP **correlated** negated sequence step `FILTER(SEQ(?e1; NOT ?e2; ?e3))` + cross-event `STR()` guards on the fast readings *and* on the negated step | Driver-attention watchdog: fast (> 80 km/h), then **no** braking reading (< 40) *by the same vehicle* before the next fast reading (> 60) → attention alert; the vehicle's own hard-brake in the gap keeps it quiet, a *different* vehicle braking in the gap no longer suppresses the alert, and two fast readings from *different* vehicles cannot pair. |
...
| [`CdspDrivingStyle`](src/main/java/org/cqels/examples/cdsp/CdspDrivingStyle.java) | Windowed two-reading self-join + `BIND(ABS(…))` + `GROUP BY`/`AVG` | The COVESA CDSP **aggressive-driving** use case: a steering swing > 210° within 3 s while moving > 10 km/h. CDSP implements this as eight RDFox Datalog rules plus an output query; here it is one continuous query, because the window replaces the temporal bookkeeping the rules hand-roll. A parked swing and a gentle correction at speed both stay silent. |
