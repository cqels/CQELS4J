...
/**
 * Example — CEP with a <em>negated</em> sequence step: {@code FILTER(SEQ(?e1; NOT ?e2; ?e3))}.
 *
 ...
 * with <em>no braking episode in between</em> — no reading ever dropped below 40 km/h. The driver
 * never reacted between the two fast readings, so the watchdog raises an attention alert.
 *
 ...
 * gap as long as none matches the negated step's condition ({@code ?s2 < 40}). A single braking
 * reading inside the gap kills the sequence. The {@code [RANGE 5s]} window doubles as the timeout
 * on a partial match. {@code BIND(?e1 AS ?v)} re-binds the first event so {@code SELECT ?v}
 * projects a bound variable — the standard projection idiom for CEP queries.
 *
 ...
 * (one vehicle fast, a different vehicle fast) that must stay quiet because the fast readings
 * cannot pair across vehicles.
 *
...
public class DriverAttentionWatchdog {
    ...
    public static void main(String[] args) throws InterruptedException {
        try (CQELSEngine engine = CQELSEngine.builder()
            ...
            engine.start();
            System.out.println("Watching for: speed > 80, then NO braking reading (< 40)"
                    + " before the next fast reading (> 60).\n");
            ...
            // -- Scenario 2: 90 -> 30 -> 75. The 30 IS a braking episode: it matches the negated
            // step (?s2 < 40) and kills the sequence, so no alert even though 90 and 75 both occur.
            ...
            pushSpeed(telemetry, Fleet.EV1, 75);
            Thread.sleep(1500);
            System.out.println("  (no alert: the braking reading matched the negated step NOT ?e2 —"
                    + " the driver reacted, so the sequence was discarded)");
